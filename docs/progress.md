...
| U1 | G2 results export (plan `2026-09-19-g2-results-export.md`; contract 52e6ce9, 920e32e, 1eafa8c reveal 409): CSV tables, YOLO/COCO labels, HTML report, unreviewed option, ONNX for other applications, Show in folder (M3) | g2-results-export (worktree `.worktrees/g2-results-export`); rounds 1-2 merged into wave1-s2-trial (efa614a) | implemented (sonnet); review (opus) + re-reviews after rounds 1 and 2; round 3 running (junction-safe partial sweep, stricter formula guard, minors); suites on the merged state green (570 backend, 410 unit, 51 e2e) | none |
