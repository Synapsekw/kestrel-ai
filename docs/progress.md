...
- 2026-09-23: **A model's training scale** on `task/model-gsd`: a model trained on imagery flown at ~191 m was being run against a 2.296 cm/px orthomosaic (~7x finer than training), finding 1.5 m of gravel instead of 9 m dump trucks. A model now carries `train_gsd_cm`; a newly trained model gets it from EXIF (median GPS altitude and sensor intrinsics over up to `EXIF_SAMPLE` of its dataset's images, cross-checked against the labelled boxes' real-world size), an existing model derives it on demand through `GET .../models/{id}/gsd-estimate` (404 with no dataset or no usable EXIF, no backfill). The New Run dialog offers the derived scale with its evidence ("trained at about 18.92 cm / px … machines 8.39 m across"), stores it once accepted, and refuses **Start detection** until a positive scale is set. A past run is the **last** resort it is specified to be: it is only consulted when nothing can be derived (the model has no dataset), so `ICVD_V4`'s own recorded 2.296 cm/px is never handed back — the whole-branch review found the dialog still prefilling it, because the derived estimate arrives asynchronously and cannot take part in the render-phase default. Detections stay visible (a minimum on-screen size) and keep their class label at every zoom, independent of the density-cell switch; the clamped mark opts out of decluttering (`declutterMode: "none"`), which otherwise dropped overlapping marks — not just labels — at exactly the zoom the clamping exists for. Also from that review: the cross-check now measures **ground truth only** (`review_state in ("accepted", "edited")`, `materialise.py`'s convention) and only classes the dataset still has, so unreviewed suggestions and deleted classes no longer drag `per_class_m`/`median_object_m`/`plausible`; and the stored-pixel divisor is the frame's **long** side, so an `exif_transpose`d portrait frame no longer estimates 1.5x too large (28.4 instead of 18.92 cm/px — inside the plausible band, so it would have been offered silently). Full gate green: contract check; ruff check + format; backend pytest **1014 passed, 9 deselected**; frontend lint (0 errors, the one pre-existing `MapView.tsx` warning); frontend unit **667 tests / 143 files**; build; **66/66 e2e** on the mock, including a test that mocks the model list, the gsd-estimate response with the imagery's real derived numbers, *and the past run at 2.296 cm/px*, then screenshots the offer panel beside the still-empty field (`cargo test` skipped: no frozen sidecar in this worktree). Evidence: `docs/evidence/model-gsd/2026-09-23-scale-offer.png`. Not yet merged to main: task 9 closes the gate and the evidence; the merge is the operator's step.

- 2026-09-26: **A model's training scale, ported onto the model library.** `main` had moved ~140 commits under `task/model-gsd` (train/detect split, app-wide model library), so the branch was ported rather than merged: `training/gsd.py` -> `library/gsd.py` (pure, unchanged), `estimate_train_gsd` -> `library/service.py` where it resolves the originating project from the library model's `provenance` snapshot (`project_folder` + `dataset_id`) and opens it the way `/usage` does, the endpoint became `GET /library/models/{id}/gsd-estimate`, and `train_gsd_cm` joined `LibraryModelPatch` - main already had the column but no way to set it. The train job now derives the scale and passes it to `add_model`, best effort: an unmeasurable or implausible run is registered without one. The dialog work moved from the deleted `maps/NewRunDialog.tsx` onto `runs/RunDialog.tsx`, where the original bug was still fully live (`placeholder="map's own"`, and an empty field omitted `target_gsd_cm` altogether): it now defaults to the model's `train_gsd_cm`, offers the derived scale with its evidence and stores it once accepted, always sends the number, and **Start run** is disabled - and the submit handler refuses - while a map run has no positive scale. The scale field also left "More options": a `Disclosure` does not render its content while closed, so a required field behind one would have disabled Start with nothing on screen to fix. Dropped as redundant: this branch's `0007_model_train_gsd.py` migration and the `train_gsd_cm` column it added to the project-scoped `Model` (main's own 0007/0008 occupy that revision and the library owns the field), and `maps/runModel.ts`'s `defaultTargetGsd`/`lastRunTargetGsd`, which nothing on main called - `defaultTargetGsd` still ended `?? mapGsd`, the default spec section 5 deletes. The spec's precedence (3), "the last run of this model on any map", is **not** carried over: main's run dialog lists sources, not runs, and fetching past runs to reinstate a last resort was not worth a request - `detect/runs.py` already falls back to the model's own scale server-side. Full gate green on the ported branch: contract check; ruff check + format; backend pytest **2007 passed, 10 skipped, 9 deselected**; frontend lint (0 errors, main's one pre-existing `MapView.tsx` warning); frontend unit **1012 tests / 204 files**; build; **95/95 e2e** on the mock (`cargo test` skipped: no frozen sidecar in this worktree). Evidence refreshed: `docs/evidence/model-gsd/2026-09-23-scale-offer.png`. Still not merged to main; the merge is the operator's step.
