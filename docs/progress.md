...
| U1 | E4 negative images (plan `2026-09-19-e4-negative-images.md`; contract 916942c) | e4-negative-images (worktree `.worktrees/e4-negative-images`); trial merge on `wave1-e4-trial` (6ea6897) | implemented (sonnet, 0afa7da..45c7217); goal-owner suites on the trial merge green (439 backend, 312 unit, 47 e2e); verified on the packaged app (a project created before E4 upgrades on open; N marks an image empty; the dataset dialog counts it as a negative); reviewed (fable): approve with fixes (store invariant, undo of the mark, migration downgrade, pre-annotation on marked images, key repeat, 10 minors); fix round 1 running | none |
| U1 | S2 Datasets screen (plan `2026-09-19-s2-datasets-screen.md`; worktree `.worktrees/s2-datasets-screen`, contract 6cb373a), then G2 results export (plan `2026-09-19-g2-results-export.md`; owner wants CSV, YOLO/COCO, HTML report, unreviewed option, ONNX for other applications; worktree `.worktrees/g2-results-export`, contract 52e6ce9 + 920e32e) | - | plans written, worktrees and contract edits prepared; each starts when its predecessor is merged and verified | none |
