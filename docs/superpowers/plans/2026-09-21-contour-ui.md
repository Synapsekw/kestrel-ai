...
- [x] Commit docs/evidence by exact paths, rebase onto main if needed, merge only the tested state,
  remove this worktree and branch with verified path/junction handling, then execute /wrapup.
- [x] Report the delivered design and numbered operator walkthrough. Clarify installer unchanged.
