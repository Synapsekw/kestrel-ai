...
(async () => {
  ...
  await page.fill('#project-name', 'Kestrel identity verification');
  ...
  const brandImage = page.getByRole('navigation').locator('img').first();
  await expect(brandImage).toBeVisible();
  await expect.poll(() => brandImage.evaluate(i => i.complete && i.naturalWidth > 0)).toBe(true);
  assert((await brandImage.boundingBox()).width >= 25, 'Compact bird receives enough space');
  assert.equal(await brandImage.getAttribute('alt'), '');
  await expect(page.getByRole('navigation').getByText('Kestrel AI', { exact: true })).toHaveCount(1);
  check('Approved local bird loads in compact branding with one accessible product name');
  await shot('01-home');
  const nav = page.getByRole('navigation');
  assert.equal((await nav.boundingBox()).width, 82);
  await page.getByRole('button', { name: 'Expand navigation', exact: true }).click();
  await expect(brandImage).toBeVisible();
  assert((await brandImage.boundingBox()).width >= 20, 'Expanded bird receives enough space');
  await expect(page.getByRole('navigation').getByText('Kestrel AI', { exact: true })).toBeVisible();
  check('Expanded branding shows the same bird and the product name');
