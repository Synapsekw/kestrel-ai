...
PLAN = {
    ...
    "image_guidance": "Use varied views.",
    "labeling_query": "Find cranes",
}
...
def sdk(monkeypatch):
    captured = {}
    state = {
        "result": {"message": "Review this plan", "plan": PLAN},
        "error": None,
        "status": "completed",
...
def test_agent_structured_plan_and_budgets(client, app, sdk, provider):
    ...
    assert response.json() == {"message": "Review this plan", "plan": PLAN, "model_name": "configured-model"}
