...
class LibraryModelPatch(BaseModel):
    ...
    #: cm of ground per model-input pixel at training (spec 2026-09-23). Null clears it back to
    #: "unknown", which is what stops a run starting at a scale nobody established.
    train_gsd_cm: float | None = Field(None, gt=0)


class ModelGsdEstimate(BaseModel):
    """A training scale derived from the model's dataset, with the evidence a human reads."""
