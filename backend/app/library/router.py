...
@router.get("/models/{modelId}/gsd-estimate", response_model=ModelGsdEstimate)
def model_gsd_estimate(
    modelId: str,  # noqa: N803
    request: Request,
    lib: LibraryHandle = Depends(get_library),
) -> ModelGsdEstimate:
    """The scale this model was trained at, derived from its dataset (spec 2026-09-23 section 3).

    404 when nothing can be measured: no dataset in the provenance snapshot, a project folder that
