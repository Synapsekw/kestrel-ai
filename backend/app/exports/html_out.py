...
def draw_thumbnail(
    path: Path, boxes: list[ExportBox], classes: list[dict], max_side: int = MAX_SIDE
) -> bytes | None:
    """Opens `path`, draws `boxes` in their class colours and names, returns a JPEG capped at `max_side`."""
    if not path.is_file():
        return None
    ...
    for box in boxes:
        ...
        draw.text((x0 + 2, max(0, y0 - 11)), name_by_id.get(box.class_id, box.class_id), fill=colour)
    buf = io.BytesIO()
...
def write(
    ...
    *,
    ...
    if thumbnail_fn is None:
        ...
        def thumbnail_fn(image: ExportImage) -> bytes | None:  # noqa: FURB118 - needs `classes` in scope
            return draw_thumbnail(root / image.path, image.boxes, classes, MAX_SIDE) if root else None

    checked = [i for i in images if i.marked_empty or i.boxes]
    ...
    parts = [
        ...
        f"<p>Exported {_e(export_time.isoformat(timespec='minutes'))}</p>",
