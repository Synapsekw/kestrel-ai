...
describe("library api", () => {
  ...
  it("derives and stores the scale a model was trained at", async () => {
    const { api, requests } = fakeClient([
      {
        method: "GET",
        path: /\/gsd-estimate$/,
        body: { train_gsd_cm: 18.92, median_alt_m: 191.02, median_object_m: 8.39, plausible: true },
      },
      {
        method: "PATCH",
        path: /\/library\/models\/[^/]+$/,
        body: { ...exampleTrainedModel, train_gsd_cm: 18.92 },
      },
    ]);
    const estimate = await fetchModelGsdEstimate(api, TRAINED_MODEL_ID);
    expect(estimate.train_gsd_cm).toBe(18.92);
    expect(requests[0].url).toBe(`/api/v1/library/models/${TRAINED_MODEL_ID}/gsd-estimate`);
    const patched = await updateLibraryModel(api, TRAINED_MODEL_ID, { train_gsd_cm: 18.92 });
    expect(patched.train_gsd_cm).toBe(18.92);
    expect(requests[1]).toMatchObject({
      method: "PATCH",
      url: `/api/v1/library/models/${TRAINED_MODEL_ID}`,
