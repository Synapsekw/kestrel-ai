...
/** Spec 2026-09-23 section 6: the one line that explains what the number is for. */
const SCALE_UNKNOWN =
...
export function RunDialog({
  ...
  // ------------------------------------------------ the scale the model was trained at (spec 2026-09-23)
  //
  // The only default is the model's own training scale. The map's own ground size is NOT a default
  // anywhere any more: it means "do not resample", which is the wrong answer for every model flown
  // at a different height, and it is what found three 1.5 m objects on a site whose dump trucks are
  // 8.9 m. It stays available as a manual entry, because running at the map's native scale is a
  // legitimate — and now explicit — choice.
  //
  // Re-prefills the field when the detector or the model changes, without fighting a value the
  ...
  const gsdKey = `${detector}|${effectiveModel}`;
  const [gsdKeySeen, setGsdKeySeen] = useState(gsdKey);
  if (gsdKey !== gsdKeySeen) {
    setGsdKeySeen(gsdKey);
    setGsd(detector === "local_model" && model?.train_gsd_cm ? String(model.train_gsd_cm) : "");
    setOffer(null);
  }

  // A real scale, not merely a non-empty string: the form has `noValidate` (Dialog.tsx), so the
  // native min="0.1" never runs, and `0` or a negative number is as unknown as an empty field.
  const gsdNum = Number(gsd);
  const scale = Number.isFinite(gsdNum) && gsdNum > 0 ? gsdNum : null;
  // Only a map run resamples, and only a local model carries a training scale: a cloud provider has
  // none to establish, and photos are read at their own resolution.
  const needsScale = hasMaps && detector === "local_model";
  const scaleMissing = needsScale && scale === null;
  // A model with no stored scale but a dataset in its provenance snapshot can be measured.
  const derivable = !!model?.provenance?.dataset_id;

  useEffect(() => {
    if (detector !== "local_model" || !model || model.train_gsd_cm || !derivable) return;
    let cancelled = false;
    fetchModelGsdEstimate(api, model.id)
      .then((e) => !cancelled && e.plausible && setOffer(e))
      .catch(() => !cancelled && setOffer(null));
    return () => {
      cancelled = true;
    };
  }, [api, detector, model, derivable]);

  /** Fill the field with the derived scale and remember it on the model, so it is asked once. */
  async function acceptOffer() {
    if (!offer || !model) return;
    setGsd(String(offer.train_gsd_cm));
    setOffer(null);
    try {
      registry.replace(await updateLibraryModel(api, model.id, { train_gsd_cm: offer.train_gsd_cm }));
    } catch {
      // The field is already filled; failing to remember it must not block this run.
    }
  }

  // The scale actually being run at, which is what the map is compared against — not merely the one
  // stored on the model, so an accepted offer or a typed value warns just as truthfully.
  const warnings =
    detector === "local_model"
      ? chosen.filter((o) => o.kind === "map").flatMap((o) => gsdWarning(o, scale) ?? [])
      : [];
  ...
  function body(): RunCreate {
    return {
      ...
      ...(hasMaps && scale !== null ? { target_gsd_cm: scale } : {}),
    };
  }
  ...
  function problem(): string | null {
    ...
    // Checked here as well as on the button: a disabled button is a hint, not a guarantee.
    if (scaleMissing) return SCALE_UNKNOWN;
    return null;
  }
    ...
    >
      {step.kind === "mapping" ? (
        ...
        <div className="flex flex-col gap-5">
          ...
          {/* Not an advanced option: a map run cannot start without it, so it is never hidden
              behind a disclosure the operator would have to go looking for. */}
          {hasMaps && (
            <div className="flex flex-col gap-3">
              <Field
                ...
                hint={
                  scaleMissing
                    ? SCALE_UNKNOWN
                    : "Maps are always read in windows, rescaled to this size before the model sees them."
                }
              >
                <Input
                  id="run-gsd"
                  ...
                  className="max-w-32"
                  aria-invalid={scaleMissing || undefined}
                />
              </Field>
              {offer && (
                <div
